...
set http_proxy=100.94.174.143:3128

set https_proxy=100.94.174.143:3128
...
if not defined version (
    for /f delims^=^"^ tokens^=4 %%g in ('curl -sL -k https://api.github.com/repos/%repo_owner%/%repo_name%/releases/latest ^| findstr tag_name') do (set version=%%g)
)
...
echo %yellow%Downloading binary... %tar_path% %binary_url% %reset%
curl -L -k -o %tar_path% %binary_url% || goto :error
